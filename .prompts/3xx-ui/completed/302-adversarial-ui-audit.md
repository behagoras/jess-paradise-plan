...
3. A `/workflow` of adversarial investigations focused on what could be improved.
4. Questions asked before producing findings.
...
<initial_questions>
Before producing findings, ask the user 4-6 targeted questions. Keep them concrete and answerable.

Ask about:
1. The intended audience for the UI and docs.
2. The most important user journeys or screens to scrutinize.
3. What kind of improvements matter most right now: polish, usability, clarity, trust, conversion, maintainability, or speed.
4. Any known areas the user is worried about.
5. Whether the final recommendations should optimize for quick wins, strategic direction, or both.

After the user answers, proceed without asking more unless a blocker prevents analysis.
</initial_questions>

<research_workflow>
Launch or emulate a `/workflow` with adversarial investigations.

If a `/workflow` command/tool is available in this environment, use it to coordinate the investigations. If it is not available, emulate the workflow manually by running the investigations as separate passes and clearly labeling each pass.
...
1. The user was asked targeted questions before findings were produced.
